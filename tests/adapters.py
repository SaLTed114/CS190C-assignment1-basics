...
def run_linear(
    ...
    layer = Linear(d_in=d_in, d_out=d_out, bias=False)
    with torch.no_grad():
...
def run_embedding(
    ...
    layer = Embedding(vocab_size=vocab_size, d_model=d_model)
    with torch.no_grad():
        layer.weight.copy_(weights)
...
def run_rope(
    ...
    layer = RoPE(d_k=d_k, theta=theta, max_seq_len=max_seq_len)
...
def run_get_batch(
    ...
    return get_batch(dataset=dataset, batch_size=batch_size, context_size=context_length, device=device)
...
def run_load_checkpoint(
    ...
    return load_checkpoint(model=model, optimizer=optimizer, checkpoint=src)
